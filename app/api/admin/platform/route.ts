...
export async function PUT(request: Request) {
  ...
  const parsed = await readJsonPayload<unknown>(
    request,
    getMaxPayloadBytes('FRANKAI_PLATFORM_ADMIN_MAX_BYTES', DEFAULT_MAX_PAYLOAD_BYTES)
  )
  if (!parsed.ok) {
...
export async function POST(request: Request) {
  ...
  const approvalPhrase =
    typeof parsed.payload.approvalPhrase === 'string' ? parsed.payload.approvalPhrase.trim() : ''
  const approvalNote =
    typeof parsed.payload.approvalNote === 'string' ? parsed.payload.approvalNote.trim() : ''
  const publishedBy =
    typeof parsed.payload.publishedBy === 'string' && parsed.payload.publishedBy.trim()
      ? parsed.payload.publishedBy.trim()
      : 'platform-admin'

  if (approvalPhrase !== APPROVAL_PHRASE || approvalNote.length < 20) {
    return NextResponse.json(
      { error: 'Approval phrase mismatch or note too short (minimum 20 characters).' },
      { status: 400 }
    )
  }
